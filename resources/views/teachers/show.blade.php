...
@section('content')
    ...
    <div class="mt-3 border border-[#E5E3DB] bg-white">
        <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
            ...
            <a href="{{ route('teachers.edit', ['id' => $teacher['id']]) }}" class="bg-[#16213A] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[#26324f]">Ubah</a>
        </div>

        <dl class="divide-y divide-[#EFEDE6] text-sm">
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase text-xs text-slate-400">NIP</dt>
                <dd class="font-medium text-[#16213A]">{{ $teacher['nip'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase text-xs text-slate-400">Nama Lengkap</dt>
                <dd class="font-medium text-[#16213A]">{{ $teacher['name'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase text-xs text-slate-400">Jenis Kelamin</dt>
                <dd class="font-medium text-[#16213A]">{{ $teacher['gender'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase text-xs text-slate-400">Mata Pelajaran</dt>
            ...
            <div class="flex justify-between px-8 py-4">
                <dt class="text-xs uppercase widest[0.1em] text-slate-400">Status</dt>
