...
@section('content')
    ...
    <div class="mt-3 border border-[#E5E3DB] bg-white">
        <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
            <div>
                <p class="mb-1 text-[11px] uppercase tracking-[0.2em] text-[#A16207]">Keterangan Kelas</p>
                <h1 class="font-display text-3xl font-semibold text-[#16213A]">{{ $class['name'] }}</h1>
                <p class="mt-1 font-mono text-xs text-slate-500">{{ $major['description'] }}</p>
            </div>
        ...
        <dl class="divide-y divide-[#EFEDE6] text-sm">
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase widest-[0.1em] text-xs text-slate-400">id</dt>
                <dd class="font-medium text-[#16213A]">{{ $class['id'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase widest-[0.1em] text-xs text-slate-400">Nama Kelas</dt>
                <dd class="font-medium text-[#16213A]">{{ $class['name'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase widest-[0.1em] text-xs text-slate-400">Tingkat</dt>
                <dd class="font-medium text-[#16213A]">{{ $class['grade'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase widest-[0.1em] text-xs text-slate-400">Jurusan</dt>
                <dd class="font-medium text-[#16213A]">{{ $major['name'] }}</dd>
            </div>
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase widest-[0.1em] text-xs text-slate-400">Wali Kelas</dt>
